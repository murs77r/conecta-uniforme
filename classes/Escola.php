...
class Escola {
    ...
    public function __construct($email, $nome, $cnpj, $senha, $cep, $estado, $cidade, $endereco, $complemento) {
        $this->email = $email;
        $this->nome = $nome;
        $this->cnpj = $cnpj;
        $this->senha = $senha;
    ...
    public function salvar() {
        ...
        try {
            if ($this->id) {
                $stmt = $con->prepare("UPDATE escola SET email=?, nome=?, cnpj=?, senha=sha2(?),cep=?, estado=?, cidade=?, endereco=?, complemento=?, cod_acesso=? WHERE id=?");
                if (!$stmt) {
                ...
                $stmt = $con->prepare("INSERT INTO escola (email, nome, cnpj, senha, cep, estado, cidade, endereco, complemento, cod_acesso) VALUES (?, ?, ?, sha2(?), ?, ?, ?, ?, ?, ?)");
                if (!$stmt) {
                    throw new Exception("Erro ao preparar statement: " . $con->error);
                }
                $stmt->bind_param(
